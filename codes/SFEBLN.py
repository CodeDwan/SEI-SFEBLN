...
def SFEBLN(x_train, y_train, x_test, y_test, x_train_ex, x_test_ex, s, C, N1, N2, N3, fftn, N4):
    L = 0
    train_x = preprocessing.scale(x_train_ex, axis=1)
    ...
    # 生成信号处理节点SP
    train_x_complex = real2complex(x_train)
    ...
    x_in = np.concatenate((x_fft, x_dct), axis=1)
    InputOfSPLayerWithBias = np.hstack(
        [x_in, 0.1 * np.ones((x_in.shape[0], 1))]
    )
    if x_in.shape[1] >= N4:
        # random.seed(67797326)
        random.seed()
        weightOfSPLayer = LA.orth(2 * random.randn(InputOfSPLayerWithBias.shape[1], N4)) - 1
    else:
        # random.seed(67797326)
        random.seed()
        weightOfSPLayer = LA.orth(2 * random.randn(InputOfSPLayerWithBias.shape[1], N4).T - 1).T
    ...
    OutputWeight = np.dot(pinvOfInput, y_train)  # 输出结果权重
    # 计算结束，终止计时
    time_end = time.time()
    ...
    #-----------------------------------------------------------------------------#
    # 测试过程，读入测试数据集
    test_x = preprocessing.scale(x_test_ex, axis=1)
    ...
    test_x_complex = real2complex(x_test)
    x_fft = sp_fft(test_x_complex, n=fftn)
    x_fft = preprocessing.scale(x_fft, axis=1)
    x_dct = sp_dct(test_x_complex, n=fftn)
    x_dct = preprocessing.scale(x_dct, axis=1)
    x_test_in = np.concatenate((x_fft, x_dct), axis=1)
    InputOfSPLayerWithBiasTest = np.hstack(
        [x_test_in, 0.1 * np.ones((x_test_in.shape[0], 1))]
    )
