...
def BLS_SP(train_x, train_y, test_x, test_y, x_sp, s, c, N1, N2, N3, fftn, N_SP):
    L = 0
    # train_x (60000,784),样本6w，每个样本784点(28*28得出)
    train_x = preprocessing.scale(train_x, axis=1)
    # 横向拼接,给偏置,FeatureOfInputDataWithBias(60000*785)
    ...
    # parameterOfShrink_SP = s / np.max(tempOfOutputOfSPLayerTest)
    OutputOfSPLayerTest = tansig(tempOfOutputOfSPLayerTest * parameterOfShrink_SP)
    # tempOutputOfSPLayer = np.concatenate((x_fft,x_dct), axis=1)
    # OutputOfSPLayerTest = tansig(tempOutputOfSPLayer)
    # tempOutputOfSPLayerTest = np.concatenate((x_mean_test, x_var_test, x_std_test,
    #                                           x_ske_test, x_kur_test, x_fft_test), axis=1)
    #
    # parameterOfShrink = s / np.max(tempOutputOfSPLayerTest)
    #
    # OutputOfSPLayerTest = tansig(tempOutputOfSPLayerTest * parameterOfShrink)

    # InputOfOutputLayerTest = np.hstack([OutputOfFeatureMappingLayerTest, OutputOfEnhanceLayerTest, OutputOfSPLayerTest])
    InputOfOutputLayerTest = np.hstack([OutputOfFeatureMappingLayerTest, OutputOfEnhanceLayerTest,
                                        OutputOfSPLayerTest])
    OutputOfTest = np.dot(InputOfOutputLayerTest, OutputWeight)
    time_end = time.time()
