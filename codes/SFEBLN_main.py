...
if __name__ == "__main__":
    ...
    datapath = '../Datasets/data_longADSB/ADS-B_200.mat'

    # 数据处理，导入，打包
    print('----------------External Data Processing----------------')
    x_train, y_train, x_test, y_test = readdata_longADSB(datapath)
    x_train_ex = sp_fft(real2complex(x_train), n=2048)
    x_test_ex = sp_fft(real2complex(x_test), n=2048)
    ...
    fftn = 32
    N4 = 100
    _, _, _, _, outputlabel = SFEBLN(x_train, y_train, x_test, y_test, x_train_ex, x_test_ex, s, C, N1, N2, N3, fftn, N4)
